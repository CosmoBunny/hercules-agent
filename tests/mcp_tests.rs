...
use mock_mcp_transport::{MockClientTransport, MockMcpServer};
...
mod regression {
    ...
    async fn test_subscription_stream_lifecycle() {
        ...
        let listen_resp = client
            ...
            .await
            .expect("subscriptions/listen should succeed");

        // Should have acknowledgment with subscription ID in _meta
        assert!(
            listen_resp.get("result").is_some(),
            "Listen should return result"
        );
        ...
        assert_eq!(
            events_received, 3,
            "Should receive exactly 3 subscription events"
        );

        println!(
            "Successfully received {} subscription events with ID: {}",
            events_received, sub_id
        );
    }
}
